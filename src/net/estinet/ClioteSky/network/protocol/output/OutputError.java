...
public class OutputError extends OutputPacket implements Packet {
	...
	public void run(List<String> args, Cliote sender) {
		try{
			...
			ClioteSky.fprintln("[Error] Error code " + args.get(0) + " being sent to " + NetworkUtil.getIP(ClioteSky.getClioteSocket(sender).getSocket()) + ":" + ClioteSky.getClioteSocket(sender).getSocket().getPort());
